...
/**
 * AdvancedPath, a {@link Path} object that contains extra paths
 *
 * @see Path
 */
public class AdvancedPath extends Path {
  ...
  public void addCircle(final float x, final float y, final float radius) {
    ...
    addArc(x, y, radius, radius, 0, 360);
  }

  /**
   * Adds to the receiver the round-cornered rectangle specified by x, y, width
   * and height.
   *
   * @param x
   *          the x coordinate of the rectangle to add
   ...
   *              </ul>
   */
  public void addRoundRectangle(final float x, final float y,
    ...
    // Top left corner
    cubicTo(x, y, x, y, x, y + arcHeight);
    cubicTo(x, y, x, y, x + arcWidth, y);
    ...
    // Bottom left corner
    cubicTo(x, y + height, x, y + height, x + arcWidth, y + height);
    cubicTo(x, y + height, x, y + height, x, (y + height) - arcHeight);
  }
   ...
   *              </ul>
   */
  public void addRoundRectangleStraightRight(final float x, final float y,
      final float width, final float height, final float arcWidth,
      final float arcHeight) {
    if (isDisposed()) {
      SWT.error(SWT.ERROR_GRAPHIC_DISPOSED);
