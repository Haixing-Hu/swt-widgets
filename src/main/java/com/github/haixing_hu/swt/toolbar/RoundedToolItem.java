...
public class RoundedToolItem extends Item {
  ...
  private static Color START_GRADIENT_COLOR = SWTResourceManager.getColor(70,
      70, 70);
  private static Color END_GRADIENT_COLOR = SWTResourceManager.getColor(116,
      116, 116);
  ...
  void drawButton(final GC gc, final int x, final int toolbarHeight,
  ...
  private void drawBackground(final int x) {
    ...
    final int cornerRadius = parentToolbar.getCornerRadius();
    if (isFirst) {
      path.addRoundRectangleStraightRight(x, 0, getWidth(), toolbarHeight,
          cornerRadius, cornerRadius);
    } else if (isLast) {
      path.addRoundRectangleStraightLeft(x, 0, getWidth(), toolbarHeight,
          cornerRadius, cornerRadius);
    } else {
    ...
    gc.fillGradientRectangle(x, 0,
        getWidth() + parentToolbar.getCornerRadius(), toolbarHeight, true);

    gc.setClipping((Rectangle) null);
  }

  private void drawRightLine(final int x) {
    gc.setForeground(parentToolbar.getBorderColor());
