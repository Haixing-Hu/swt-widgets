...
import com.github.haixing_hu.swt.utils.ReadOnlyStyledText;
...
public class MessageArea extends DialogArea {
  ...
  public void render() {
    ...
    this.composite.setLayoutData(new GridData(GridData.FILL, GridData.FILL,
        true, true));
    this.composite.setBackground(this.composite.getDisplay().getSystemColor(
        SWT.COLOR_WHITE));

    final boolean hasIcon = this.icon != null;
    final boolean hasTitle = ! StringUtils.isEmpty(this.title);
    final boolean hasText = ! StringUtils.isEmpty(this.text);
  ...
  private void createIcon(final int numberOfRows) {
    ...
    label.setBackground(this.composite.getDisplay().getSystemColor(
        SWT.COLOR_WHITE));
    label.setLayoutData(new GridData(GridData.CENTER, GridData.BEGINNING,
        false, false, 1, numberOfRows));
  }
  ...
  private void createTitle(final boolean hasIcon) {
    ...
    final GridData gd = new GridData(GridData.BEGINNING, GridData.BEGINNING,
        false, false, 1, 1);
  ...
  private void createText(final boolean hasIcon, final boolean hasTitle) {
    ...
    this.label = new ReadOnlyStyledText(this.composite, SWT.NONE
        | (this.verticalScrollbar ? SWT.V_SCROLL : SWT.NONE));
    this.label.setText(this.text);
    SWTUtils.applyHTMLFormating(this.label);
  ...
  private void createTextException() {
    this.textException = new Text(this.composite, SWT.MULTI | SWT.READ_ONLY
        | SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL);
    final StringWriter writer = new StringWriter();
    exception.printStackTrace(new PrintWriter(writer));
    this.textException.setText(writer.toString());
    final Display disp = Display.getCurrent();
    this.textException.setBackground(disp.getSystemColor(SWT.COLOR_WHITE));
    final GridData gd = new GridData(GridData.FILL, GridData.FILL,
                                     true, true, 1, 1);
  ...
  private void createProgressBar() {
    ...
    this.progressBar.setBackground(Display.getCurrent().getSystemColor(
        SWT.COLOR_WHITE));
