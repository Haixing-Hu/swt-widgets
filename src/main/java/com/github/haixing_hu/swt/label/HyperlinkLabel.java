...
public class HyperlinkLabel extends Canvas {
  ...
  public HyperlinkLabel(Composite parent, int style) {
    ...
    final Listener mouseListener = new Listener() {
      @Override
      public void handleEvent(Event event) {
        final boolean newCursorInControl = isInClientArea(event);
  ...
  public Point computeSize(int wHint, int hHint, boolean changed) {
    checkWidget();
    final Point e = getTotalSize(text);
  ...
  private Point getTotalSize(String text) {
    ...
    if ((text != null) && (text.length() > 0)) {
      final Point e = gc.textExtent(text, SWT.DRAW_MNEMONIC);
      size.x += e.x;
  ...
  private void onPaint(PaintEvent event) {
    final Rectangle rect = cachedClientArea; // getClientArea();
    if ((rect.width == 0) || (rect.height == 0)) {
      return;
    }

    final Point extent = getTotalSize(text);
    ...
    final int uy = (rect.y + ((rect.height - textHeight) / 2))
        + gc.getFontMetrics().getAscent() + gc.getFontMetrics().getLeading()
        + 1;
    final int lineWidth = extent.x > rect.width ? rect.width : extent.x;
